		...
		var table = $('#reservationTable').DataTable( {
	        ...
	        columns: [
				...
				{ data: function(callback){
					...
					{
						return `
							<button data-id="`+callback.id+`" class="approve btn btn-xs btn-success">Approve</button>
							<button data-id="`+callback.id+`" data-reason="`+callback.remark+`"  class="disapprove btn btn-xs btn-danger">Disapprove</button>
						`
					}
					if(callback.approval == 1)
					{
