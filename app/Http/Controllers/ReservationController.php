...
class ReservationController extends Controller {
	...
	{		
		...
		*/
		...
		|--------------------------------------------------------------------------
		|
		| 	date of use must not be in the events list
		|
		|--------------------------------------------------------------------------
		|
		*/
		if(SpecialEvent::isAvailable($dateofuse) == false)
		{
			Session::flash('error-message','Date is not available for reservation');
			return redirect('reservation/create')
					->withInput()
					->withErrors(['Date is not available for reservation']);
		...
		*/
		if(Auth::user()->accesslevel != 0 && Auth::user()->accesslevel != 1 )
		{
			...
			|
			*/
			if(!Carbon::parse($dateofuse)->isSameDay(Carbon::now()->addDays(3)))
			{
				Session::flash('error-message','Reservation must occur 3 days before day of usage ');
				return redirect('reservation/create')
						->withInput()
						->withErrors(['Reservation must occur 3 days before day of usage']);
			}
		}
		...
		if(Request::ajax())
		{
			...
			*/
			if(count($reservation) > 0)
			{
		        ...
		        Mail::send('reservation.create', ['user'=>$user], function ($message) use ($subject,$user) {
